    ...
    public class AssemblyRepository
    {
        ...
        public AssemblyRepository(string? connectionString = null, string? masterConnectionString = null)
        {
            _connectionString = connectionString ?? ConfigurationManager.ConnectionStrings["AssemblyDb"]?.ConnectionString
                ?? "Data Source=(localdb)\\MSSQLLocalDB;Initial Catalog=AssemblyManager;Integrated Security=True;";
        ...
        public async Task<List<AssemblyRecord>> GetAssembliesAsync()
        {
            ...
            {
                cmd.CommandText = @"
                    SELECT Id, AssemblyId, Name, PartNumber, Material, Quantity, ModelPath, Notes
                    FROM dbo.Parts
                    ORDER BY PartNumber;";
        ...
        public async Task<AssemblyRecord> SaveAssemblyAsync(AssemblyRecord assembly)
        {
            ...
            if (assembly.Id == 0)
            {
                ...
                insert.CommandText = @"
                    INSERT INTO dbo.Assemblies (Name, Code, Description, CreatedAt, UpdatedAt)
                    OUTPUT INSERTED.Id
                    VALUES (@name, @code, @description, @createdAt, @updatedAt);";
                insert.Parameters.AddWithValue("@name", assembly.Name);
                insert.Parameters.AddWithValue("@code", (object?)NullIfEmpty(assembly.Code) ?? DBNull.Value);
                insert.Parameters.AddWithValue("@description", (object?)assembly.Description ?? DBNull.Value);
            ...
            else
            {
                ...
                update.CommandText = @"
                    ...
                    SET Name = @name, Code = @code, Description = @description, UpdatedAt = @updatedAt
                    WHERE Id = @id;";
                update.Parameters.AddWithValue("@name", assembly.Name);
                update.Parameters.AddWithValue("@code", (object?)NullIfEmpty(assembly.Code) ?? DBNull.Value);
                update.Parameters.AddWithValue("@description", (object?)assembly.Description ?? DBNull.Value);
        ...
        public async Task<PartRecord> SavePartAsync(int assemblyId, PartRecord part)
        {
            ...
            if (part.Id == 0)
            {
                ...
                using var insert = connection.CreateCommand();
                insert.CommandText = @"
                    INSERT INTO dbo.Parts (AssemblyId, Name, PartNumber, Material, Quantity, ModelPath, Notes)
                    OUTPUT INSERTED.Id
                    VALUES (@assemblyId, @name, @partNumber, @material, @quantity, @modelPath, @notes);";
                insert.Parameters.AddWithValue("@assemblyId", assemblyId);
                insert.Parameters.AddWithValue("@name", part.Name);
                insert.Parameters.AddWithValue("@partNumber", (object?)NullIfEmpty(part.PartNumber) ?? DBNull.Value);
                insert.Parameters.AddWithValue("@material", (object?)part.Material ?? DBNull.Value);
                insert.Parameters.AddWithValue("@quantity", part.Quantity <= 0 ? 1 : part.Quantity);
                insert.Parameters.AddWithValue("@modelPath", (object?)part.ModelPath ?? DBNull.Value);
                insert.Parameters.AddWithValue("@notes", (object?)part.Notes ?? DBNull.Value);
            ...
            else
            {
                ...
                update.CommandText = @"
                    ...
                    WHERE Id = @id;";
                update.Parameters.AddWithValue("@name", part.Name);
                update.Parameters.AddWithValue("@partNumber", (object?)NullIfEmpty(part.PartNumber) ?? DBNull.Value);
                update.Parameters.AddWithValue("@material", (object?)part.Material ?? DBNull.Value);
                update.Parameters.AddWithValue("@quantity", part.Quantity <= 0 ? 1 : part.Quantity);
                update.Parameters.AddWithValue("@modelPath", (object?)part.ModelPath ?? DBNull.Value);
                update.Parameters.AddWithValue("@notes", (object?)part.Notes ?? DBNull.Value);
        ...
        private async Task<PartRecord> InsertPartInternalAsync(SqlConnection connection, SqlTransaction transaction, PartRecord part)
        {
            using var insert = connection.CreateCommand();
            insert.Transaction = transaction;
            insert.CommandText = @"
